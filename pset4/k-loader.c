                                ...
                                x86_64_pagetable *(*allocator)(void))
{
    ...
    {
        if (assign_physical_page(addr, p->p_pid) < 0 || virtual_memory_map(p->p_pagetable, addr, addr, PAGESIZE, PTE_P | PTE_W | PTE_U, allocator) < 0)
        {
    ...
    if ((ph->p_flags & ELF_PFLAG_WRITE) == 0)
    {
        ...
        if (vam.pn > -1)
        {
            ...
            {
                virtual_memory_map(p->p_pagetable, page_va, page_va, PAGESIZE,
